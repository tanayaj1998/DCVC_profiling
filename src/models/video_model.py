...
class DMC(CompressionModel):
    ...
    def compress(self, x, dpb, mv_y_q_scale, y_q_scale):
        ...
        print("motion estimation :", me_time)
        ...
        enc_dec_time = time.time() - enc_dec_start
        print("Contextual encoder-decoder:", enc_dec_time)
        ...
        result = {
            ...
            "mv_enc_dec" : mv_time,
            "motion_estimator_time": me_time,
            
            "main_enc_dec_time": enc_dec_time,
        }
        return result
    ...
    def decompress(self, dpb, string, height, width,
        ...
        ent_end= time.time()-ent_start
        print("entropy coding:",ent_end)
        #mv decoder 
        mv_dec_start = time.time()
        mv_y_hat = mv_y_hat * curr_mv_y_q
        
        mv_hat = self.mv_decoder(mv_y_hat)
        mv_dec_end = time.time() - mv_dec_start
        print("mv_dec:",mv_dec_end)
        #warp 
        w_start = time.time()
        context1, context2, context3, _ = self.motion_compensation(dpb, mv_hat)
        w_time = time.time()-w_start
        print("warping  decompress: ", w_time)
        #entropy 
        e_start = time.time()
        ...
        hierarchical_params = self.contextual_hyper_prior_decoder(z_hat)
        temporal_params = self.temporal_prior_encoder(context3)
        entropy_end = time.time()- e_start
        print("decompress : enrtropy coding :", entropy_end)
        ref_y = dpb["ref_y"]
        if ref_y is None:
        ...
        print("contextual decoding : ", end_cd )
    ...
    def encode_decode(self, x, dpb, output_path=None,
        ...
        if output_path is not None:
            ...
            print("decompress time:", decoding_time)
            result = {
                "dpb": decoded["dpb"],
                "bit": bits,
    ...
    def forward_one_frame(self, x, dpb, mv_y_q_scale=None, y_q_scale=None):
        ...
        me_end = time.time()-me_start
        print("motion estimation:", me_end)
        #mv enc-dec
        mv_start = time.time()
        mv_y = self.mv_encoder(est_mv)
        mv_y = mv_y / curr_mv_y_q
        ...
        mv_hat = self.mv_decoder(mv_y_hat)
        mv_end = time.time() - mv_start 
        print("mv encoder-decoder:", mv_end )
        warp_start = time.time()
        context1, context2, context3, warp_frame = self.motion_compensation(dpb, mv_hat)
        warp_end= time.time() - warp_start

        print("warp time: " ,warp_end)
        ...
        recon_image_feature = self.contextual_decoder(y_hat, context2, context3)
        feature, recon_image = self.recon_generation_net(recon_image_feature, context1)
        contextual_end = time.time() - contextual_start
        print("contextual time:",contextual_end)
